...
export function Edite({ isOpen, onClose, onSuccess, data }: Props) {
    ...
    useEffect(() => { if (data) setForm({ libelle: data.libelle ?? "", prix_vente: String(data.prix_vente ?? ""), categorie_id: String(data.categorie?.id ?? "") }); }, [data]);
    useEffect(() => { if (isOpen) apiFetch("/categories/magasin").then(res => setCategories(Array.isArray(res.data) ? res.data : res.data?.data ?? [])).catch(() => { }); }, [isOpen]);

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault(); setIsSubmitting(true);
        try {
            await apiFetch(`/services/${data.id}/update`, { method: "PUT", data: { ...form, prix_vente: parseFloat(form.prix_vente) } });
            ...
            <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
                <div className="bg-gradient-to-r from-[#0052cc] to-[#1a66b3] p-6 text-white">
                    <DialogHeader><DialogTitle className="text-xl font-bold flex items-center gap-3">
                        <div className="bg-white/20 p-2 rounded-lg"><Edit3 className="h-5 w-5" /></div>Modifier le service
