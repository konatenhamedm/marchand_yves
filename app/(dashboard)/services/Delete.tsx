...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/services/delete/${id}`, { method: "DELETE" })));
        toast.success("Services supprimées !");
      } else {
        await apiFetch(`/services/delete/${data.id}`, { method: "DELETE" });
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> services sélectionnés ?
          </p>
        ) : (
          <p className="text-slate-600">
