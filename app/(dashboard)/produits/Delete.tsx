...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/produits/delete/${id}`, { method: "DELETE" })));
        toast.success("Produits supprimés !");
      } else {
        await apiFetch(`/produits/delete/${data.id}`, { method: "DELETE" });
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> produits sélectionnés ?
          </p>
        ) : (
          <p className="text-slate-600">
