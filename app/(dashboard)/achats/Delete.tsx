...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [] }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/achats/${id}/delete`, { method: "DELETE" })));
        toast.success("Achats supprimés !");
      } else {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
        ...
        <div className="p-6 bg-white">
          {multiple ? (
            <p className="text-slate-600">Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> achats sélectionnés ?</p>
          ) : (
