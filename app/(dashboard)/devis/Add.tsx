...
import { FileText, User, Calendar, Plus, Trash2, Package, Search, Calculator, CheckCircle2 } from "lucide-react";
...
export function Add({ isOpen, onClose, onSuccess }: Props) {
    ...
    const [form, setForm] = useState({
        ...
        date_expiration: new Date(new Date().setMonth(new Date().getMonth() + 1)).toISOString().split('T')[0], // 1 month 
        client_id: "",
        commentaire: "",
        lignes_produits: [] as any[], // { produit_id, quantite, prix_unitaire, detail }
        montant_remise: 0,
    });
    ...
    const totals = useMemo(() => {
        const totalHT = form.lignes_produits.reduce((acc, l) => acc + (l.quantite * l.prix_unitaire), 0);
    ...
    const handleRemoveArticle = (idx: number) => {
        setForm(f => ({
            ...f,
            lignes_produits: f.lignes_produits.filter((_, i) => i !== idx)
        }));
    };

    const handleSubmit = async () => {
        if (!form.date_devis) return toast.error("La date du devis est requise");
        if (!form.date_expiration) return toast.error("La date d'expiration est requise");
        if (!form.client_id) return toast.error("Le client est requis");
        if (form.lignes_produits.length === 0) return toast.error("Veuillez ajouter au moins un article");
        ...
        try {
            const payload = {
                ...
                lignes_produits: form.lignes_produits.map(l => ({
            ...
            await apiFetch("/devis/create", { method: "POST", data: payload });
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-bold tracking-tight">Nouveau Devis</h2>
                </div>
            }
            size="2xl"
            footer={
                <div className="flex justify-between w-full">
                    ...
                    <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-[#0052cc] hover:bg-[#0041a8] rounded-2xl px-10 gap-2">
                        {isSubmitting ? "Création..." : "Enregistrer"} <CheckCircle2 className="w-4 h-4" />
                    </Button>
                </div>
            }
        >
            <div className="px-2 space-y-6 pb-6">
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <Button onClick={handleAddArticle} disabled={!selectedArticleId} className="h-11 rounded-xl bg-[#0052cc]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        <Label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                            <Calculator className="w-3.5 h-3.5" /> Remise Exceptionnelle
                        </Label>
                    ...
                    <div className="space-y-2 flex flex-col items-end justify-center pt-2">
                        ...
                        <div className="text-2xl font-black text-[#0052cc]">Net TTC: {totals.totalTTC.toLocaleString()} <span className="text-sm text-slate-400">XOF</span></div>
                    </div>
                </div>
