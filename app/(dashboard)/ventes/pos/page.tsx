...
import { Add as AddClientModal } from "@/app/(dashboard)/clients/Add";
...
function PosPageContent() {
    ...
    const [printData, setPrintData] = useState<any>(null); // For receipt print modal
    ...
    useEffect(() => {
        if (magasinId) {
            ...
            // 💾 Restauration du caddie sauvegardé !
            const savedCartStr = localStorage.getItem(`moomen_pos_cart_${magasinId}`);
            let initialCart = [];
            if (savedCartStr) {
                try { initialCart = JSON.parse(savedCartStr); } catch(e) {}
            }
            
            setForm(f => ({ ...f, date_vente: getInitialDateTime(), lignes_vente_produits: initialCart }));
            setBarcodeQuery("");
    ...
    useEffect(() => {
        const handleGlobalKeyDown = (e: KeyboardEvent) => {
            if (isScanning || printData) return;
    ...
    }, [isScanning, printData]);

    // 💾 Persistance automatique du panier
    useEffect(() => {
        if (magasinId) {
            localStorage.setItem(`moomen_pos_cart_${magasinId}`, JSON.stringify(form.lignes_vente_produits));
        }
    }, [form.lignes_vente_produits, magasinId]);
    ...
    const executeScan = async (codeRaw: string) => {
        ...
        // Vider et refocus immédiatement
        setBarcodeQuery("");
    ...
    const handleScanSubmit = async (e: React.FormEvent) => {
        ...
        await executeScan(codeRaw);
    };

    // Lecture du paramètre d'URL `scan` pour le scan initial
    useEffect(() => {
        const paramScan = searchParams.get('scan');
        if (paramScan && magasinId && !initialScanHandled.current) {
            initialScanHandled.current = true; // Marqué comme traité
            executeScan(paramScan);
            // NB: On ne fait PLUS de router.replace car Next.js peut violemment annuler/recharger la page en cours de calcul !
        }
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            ...
            // Vide le panier intelligemment en cas de succès absolu
            localStorage.removeItem(`moomen_pos_cart_${magasinId}`);
            
            // Vider les champs pour rester sur la même page
            setForm(f => ({
                ...f,
                date_vente: getInitialDateTime(),
                client_id: "",
            ...
            // Ouvre la modale du ticket
            setPrintData(createdData);

            // Remet le focus sur le champ de code-barre
            setTimeout(() => {
                barcodeInputRef.current?.focus();
            }, 300);
        } catch (err: any) {
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-100 overflow-hidden relative">
            ...
            {isAddClientOpen && (
                <AddClientModal 
                    ...
                    onSuccess={() => {
                        ...
                        apiFetch(`/clients/all/magasin/${magasinId}`).then(c => {
                            const newClients = Array.isArray(c.data) ? c.data : c.data?.data ?? [];
                            setClients(newClients);
                            // Auto-selection du client fraîchement créé (le dernier ajouté ou ID max)
                            if (newClients.length > 0) {
                                const latestClient = newClients.reduce((prev: any, current: any) => (prev.id > current.id) ? prev : current);
                                setForm(f => ({ ...f, client_id: String(latestClient.id) }));
                            }
                        });
                    }} 
                />
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-[#0052CC] text-white shadow-xl shadow-[#0052CC]/15 z-10 flex-shrink-0">
                ...
                <div className="flex items-center gap-4">
                     <div className="text-right">
                         <p className="text-[10px] font-black uppercase text-indigo-200">Total à encaisser</p>
                         <p className="text-2xl font-black text-white">{totals.totalTTC.toLocaleString()} XOF</p>
                     </div>
                     <div className="h-10 w-px bg-white/20 mx-2"></div>
                     <Button variant="ghost" onClick={() => { localStorage.removeItem(`moomen_pos_cart_${magasinId}`); router.push('/ventes'); }} className="text-white hover:bg-white/20 font-bold uppercase tracking-wide">Annuler</Button>
                     <Button onClick={handleFinalSubmit} disabled={isSubmitting || form.lignes_vente_produits.length === 0} className="bg-green-500 hover:bg-green-600 text-white font-black uppercase tracking-wide shadow-lg shadow-green-900/20">
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden p-4 gap-4 max-w-[1600px] mx-auto w-full">
                                            ...
                                            <tr key={trueIdx} className={`border-b border-slate-50 transition-colors ${idx === 0 ? "bg-indigo-50/50" : ""}`}>
                                                ...
                                                <td className="py-4 text-center">
                                                    <input 
                                                        ...
                                                        value={item.prix}
                                                        onChange={(e) => {
                                                            const newPu = parseFloat(e.target.value) || 0;
                                                            setForm(f => {
                                                                const nl = [...f.lignes_vente_produits];
                                                                nl[trueIdx].prix = newPu;
                                                                return { ...f, lignes_vente_produits: nl };
                                                            });
                                                        }}
                                                        onFocus={(e) => e.target.select()}
                                                        className="w-24 h-8 text-right font-black text-slate-700 border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400 px-2 transition-all shadow-inner"
                                                    />
                                                </td>
                                                <td className="py-4 text-right font-black text-red-700 text-lg">{(item.prix * item.quantite).toLocaleString()}</td>
                                                <td className="py-4 text-right pl-4">
                                                    <Button variant="ghost" onClick={() => removeLigne(trueIdx)} className="text-red-500 bg-red-50 hover:text-red-600 hover:bg-red-100 p-2 rounded-xl shadow-sm transition-colors">
                ...
                {/* R - CHECKOUT FORM */}
                <div className="w-full lg:w-[450px] bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col p-6 overflow-y-auto space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <button type="button" onClick={() => setIsAddClientOpen(true)} className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-[#0052CC] hover:text-white bg-indigo-50 hover:bg-[#0052CC] px-2 py-1 rounded-lg transition-all shadow-sm">
                                    <Plus className="w-3 h-3" /> Nouveau
                                </button>
                            </div>
                            <Select
                                instanceId="pos-client-select"
                                options={clients.map(c => ({ value: c.id, label: `${c.nom} ${c.tel}` }))}
                                value={form.client_id ? { value: form.client_id, label: clients.find(c => String(c.id) === String(form.client_id)) ? `${clients.find(c => String(c.id) === String(form.client_id))?.nom} ${clients.find(c => String(c.id) === String(form.client_id))?.tel}` : "" } : null}
                                   ...
                                   <>
                                      <div className="flex gap-4">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            ...
                                            <Select
                                                instanceId="pos-paiement-select"
                                      ...
                                      <div className="pt-2 border-t border-slate-100">
                                         ...
                                         <Input 
                                            ...
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault();
                                                    if (!isSubmitting && form.lignes_vente_produits.length > 0) {
                                                        handleFinalSubmit();
                                                    }
                                                }
                                            }}
                                            className="h-14 mt-1 text-2xl font-black text-right text-green-700 bg-green-50 border-green-200 rounded-xl"
                                        />
...
export default function PosPage() {
    return (
        <Suspense fallback={<div className="flex h-screen items-center justify-center bg-slate-100 font-bold text-[#0052CC]">Chargement de la caisse...</div>}>
            <PosPageContent />
        </Suspense>
    );
}
