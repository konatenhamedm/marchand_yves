    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                ...
                <PageHeader
                    title="Ventes"
                    description="Historique de toutes les ventes du magasin"
                ...
                >
                    <table className="w-full">
                        <TableHeaderCustom
                            items={["#", "Référence", "Date", "Client", "Montant TTC", "Réglé", "Crédit", "Statut"]}
                            afficheAction={true}
                            actionWidth="80px"
                        />
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucune vente trouvée" icon={<ShoppingCart className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100">
                                        ...
                                        <td className={TD.base}>{fmtDate(item.date_vente)}</td>
                                        <td className={TD.bold}>{item.client?.nom ?? <span className="text-slate-300 italic font-normal">Anonyme</span>}</td>
                                        <td className={TD.mono}>{fmt(item.montant_ttc)}</td>
                                        ...
                                        <td className={TD.base}>
                                            {item.status_commande ? (
                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[item.status_commande] ?? "bg-slate-50 text-slate-600"}`}>
                                                    {item.status_commande}
                                                </span>
                                            ) : "—"}
                                        </td>
                                        <td className={TD.action}>
                                            <div className="flex items-center justify-center gap-1">
                                                <button onClick={() => handleOpenModal("view", item)} title="Voir le détail"
