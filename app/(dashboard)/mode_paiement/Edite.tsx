...
export function Edite({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    useEffect(() => { if (data) setLibelle(data.libelle ?? ""); setDescription(data.description ?? ""); }, [data]);

    const handleSubmit = async (e?: React.FormEvent) => {
        if (e) e.preventDefault();
        setIsSubmitting(true);
        try {
            await apiFetch(`/modePaiements/update`, { method: "POST", data: { id: data.id, libelle ,description:data.description} });
    ...
    return (
        ...
        >
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-4">
                ...
                <div className="space-y-2">
                    <Label className="text-[#0052cc] font-semibold">Description</Label>
                    <Input value={description} onChange={e => setDescription(e.target.value)} required className="border-[#0052cc]/30 focus:border-[#0052cc] rounded-xl" />
                </div>
            </form>
        </Modal>
    );
