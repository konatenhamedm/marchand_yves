...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/modePaiements/delete/${id}`, { method: "DELETE" })));
        toast.success("Modes de paiement supprimés !");
      } else {
        await apiFetch(`/modePaiements/delete/${data.id}`, { method: "DELETE" });
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> modes de paiement sélectionnés ?
          </p>
        ) : (
          <p className="text-slate-600">
