...
export function Add({ isOpen, onClose, onSuccess }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await apiFetch("/charges/create", { method: "POST", data: { libelle: form.libelle, montant: parseFloat(form.montant) } });
            toast.success("Charge créée !"); setForm({ libelle: "", montant: "" }); onSuccess(); onClose();
            ...
            <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
                    <div className="space-y-2">
                        ...
                        <Input value={form.libelle} onChange={e => setForm(p => ({ ...p, libelle: e.target.value }))} placeholder="Ex: Loyer, Salaires..." required className="border-[#0052cc]/30 focus:border-[#0052cc] rounded-xl" />
                    </div>
                    <div className="space-y-2">
                        <Label className="text-[#0052cc] font-semibold">Montant budgété</Label>
                        <Input type="number" min="0" value={form.montant} onChange={e => setForm(p => ({ ...p, montant: e.target.value }))} placeholder="200000" required className="border-[#0052cc]/30 focus:border-[#0052cc] rounded-xl" />
                    </div>
