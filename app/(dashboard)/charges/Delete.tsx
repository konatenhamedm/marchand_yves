...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/charges/${id}/delete`, { method: "DELETE" })));
        toast.success("Charges supprimées !");
      } else {
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> charges sélectionnées ?
          </p>
        ) : (
          <p className="text-slate-600">
