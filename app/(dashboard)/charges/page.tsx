  ...
  return (
    <ClientOnly>
      <div className="space-y-5">
        ...
        <PageHeader
          title="Charges"
          description="Charges récurrentes du magasin (loyer, salaires…)"
          ...
          <table className="w-full">
            ...
            <tbody>
              {isLoading ? (
                ...
                <EmptyState message="Aucune charge trouvée" icon={<TrendingUp className="w-10 h-10" />} cols={COLS} />
              ) : (
                currentitems.map((item, index) => (
                  <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100">
                    ...
                    <td className={TD.mono}>{item.montant?.toLocaleString("fr-FR") ?? "0"} FCFA</td>
