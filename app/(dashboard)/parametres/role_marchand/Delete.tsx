...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (multiple) {
                await Promise.all(selectedIds.map(id => apiFetch(`/roles/marchands/${id}/delete`, { method: "DELETE" })));
                toast.success("Rôles marchands supprimés !");
            } else {
            ...
            <div className="py-2">
                {multiple ? (
                    <p className="text-slate-600">
                        Êtes-vous sûr de vouloir supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> rôles sélectionnés ?
                    </p>
                ) : (
                    <p className="text-slate-600">
