...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/personnels/delete/${id}`, { method: "DELETE" })));
        toast.success("Employés supprimés !");
      } else {
        await apiFetch(`/personnels/delete/${data.id}`, { method: "DELETE" });
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> employés sélectionnés ?
          </p>
        ) : (
          <p className="text-slate-600">
