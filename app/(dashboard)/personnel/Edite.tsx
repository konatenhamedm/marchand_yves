...
export function Edite({ isOpen, onClose, onSuccess, data }: Props) {
    ...
    useEffect(() => { if (data) setForm({ nom: data.nom ?? "", prenoms: data.prenoms ?? "", tel: data.tel ?? "", email: data.email ?? "", role_marchand_id: String(data.role?.id ?? "") }); }, [data]);
    useEffect(() => {
        if (isOpen) apiFetch("/roles/marchands/all").then(res => setRoles(Array.isArray(res.data) ? res.data : res.data?.data ?? [])).catch(() => { });
    }, [isOpen]);
            ...
            <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
                <div className="bg-gradient-to-r from-[#0052cc] to-[#1a66b3] p-6 text-white">
                    <DialogHeader><DialogTitle className="text-xl font-bold flex items-center gap-3">
                        <div className="bg-white/20 p-2 rounded-lg"><Edit3 className="h-5 w-5" /></div>Modifier l'employé
