...
export default function ClientsPage() {
    const { canCreate, canEdit, canDelete } = usePermissions("gestionClients");
  ...
  const refreshData = () => {
    if (!magasinId) return;
    setIsLoading(true);
    apiFetch(`/clients/all/magasin/${magasinId}?per_page=${itemsPerPage}&page=${currentPage}`)
      .then((res) => {
  ...
  const filteredData = getFilteredData(searchTerm, ["nom", "tel", "adresse", "email"]);
          ...
          <table className="w-full">
            <TableHeaderCustom
              items={[
                ...
                />,
                "Client & Contact", "Coordonnées", "Adresse Géographique"
              ]}
              afficheAction={true}
              actionWidth="160px"
            />
