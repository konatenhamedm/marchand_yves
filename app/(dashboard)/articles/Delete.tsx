...
export function Delete({ isOpen, onClose, onSuccess, data, multiple = false, selectedIds = [], size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (multiple) {
        await Promise.all(selectedIds.map(id => apiFetch(`/articles/delete`, { method: "POST", data: { ids: [id] } as any })));
        toast.success("Articles supprimés !");
      } else {
        await apiFetch(`/articles/delete`, { method: "POST", data: { ids: [data?.id] } as any });
      ...
      <div className="py-2">
        {multiple ? (
          <p className="text-slate-600">
            Voulez-vous supprimer les <span className="font-bold text-red-600">{selectedIds.length}</span> articles sélectionnés ?
          </p>
        ) : (
          <p className="text-slate-600">
